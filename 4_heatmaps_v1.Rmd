...
cts.full <-
  as.matrix(read.csv(
    "assets/counts-batch-corrected-v5_tpm.txt",
    sep = "\t",
    row.names = "gene"
  ))
md.full <-
  read.csv(
    "assets/coldata-batch-corrected-v5.txt",
    sep = "\t",
    header = TRUE,
    row.names = 1
  )
...
te.results.split <- as.data.frame(assay(output))
write.table(
  te.results.split,
  file = "enrichment-results_fc10_split.tsv",
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)

full.split <- as.data.frame(assay(output))
amnion.roost <- subset(full.split, Tissue == 'amnion')
amnion.suzuki <- subset(full.split, Tissue == 'amnion_Term')
```
...
runheatmap <-
  function(get_table = table,
    ...
    annotation <- data.frame(Condition = colnames(temp))
    ...
    annotation$sample[which(str_detect(annotation$Condition, "CT"))] <-
      "CT"
    annotation$sample[which(str_detect(annotation$Condition, "amnion"))] <-
      "Amnion"
    annotation$sample[which(str_detect(annotation$Condition, "pBAP_D3"))] <-
      "pBAP_D3"
    annotation$sample[which(str_detect(annotation$Condition, "hESC_H9_BMP4_72h"))] <-
      "BMP4_D3"
    annotation$sample[which(str_detect(annotation$Condition, "H1_gt70_D8_BAP"))] <-
      "BAP_Yabe"
    
    annotation <- annotation %>%
      arrange(levels = Condition)
    hmap.data <- as.matrix(norm.filtered.subset.table[, c(
      "amnion_9w.1",
      "amnion_9w.2",
      "amnion_16w.2",
      "amnion_18w.1",
...
```{r tableFun, warnings=TRUE, message=FALSE}
getTable <- function(genelist = df) {
  norm.filtered.table <- normalized_counts %>% filter(gene %in% genelist$ensembl_gene_id_version)
  norm.filtered.long <- melt(norm.filtered.table, id.vars="gene")
  defile= "assets/datasets_to_plot.v2.txt"
  decoldata <- read.csv(defile, sep="\t", stringsAsFactors = TRUE)
  de_names <- decoldata$name
  norm.filtered.subset.long <- norm.filtered.long %>% filter (variable %in% de_names)
  colnames(norm.filtered.subset.long) <- c("gene", "condition", "norm.expression")
  norm.filtered.subset.long <- merge(norm.filtered.subset.long, annot,
                                     by.x = "gene",
                                     by.y = "ensembl_gene_id_version",
                                     all.x = TRUE,
                                     all.y = FALSE) %>% drop_na() %>% filter(external_gene_name != "")
  norm.filtered.subset.long
}
```

```{r corplotFun, warnings=TRUE, message=FALSE}  
  corPlot <- function(get_table = table, ChartTitle = "Title") {
  norm.filtered.subset.long <- get_table
  norm.filtered.subset.table <- dcast(norm.filtered.subset.long, external_gene_name ~ condition, value.var="norm.expression")
  temp <- norm.filtered.subset.table[,-1]
  row.names(temp) <- norm.filtered.subset.table[,1]
  norm.filtered.subset.table <- temp
...
```{r normFun, warnings=TRUE, message=FALSE}
  normTestEach <- function(get_table = table, ChartTitle = "Title") {
  norm.filtered.subset.long <- get_table
...
```{r normAllFun, warnings=TRUE, message=FALSE}
  normTestAll <- function(get_table = table, ChartTitle = "Title") {
  norm.filtered.subset.long <- get_table 
  ggplot(norm.filtered.subset.long, aes(x = norm.expression)) + 
...
addSampleInfo <- function(get_table = table) {
  norm.filtered.subset.long <- get_table
  norm.filtered.subset.long$sample <- "NA"
  norm.filtered.subset.long$sample[which(str_detect(norm.filtered.subset.long$condition, "CT"))] <-
    "CT"
  norm.filtered.subset.long$sample[which(str_detect(norm.filtered.subset.long$condition, "amnion"))] <-
    "Amnion"
  norm.filtered.subset.long$sample[which(str_detect(norm.filtered.subset.long$condition, "pBAP_D3"))] <-
    "pBAP_D3"
  norm.filtered.subset.long$sample[which(str_detect(norm.filtered.subset.long$condition, "hESC_H9_BMP4_72h"))] <-
    "BMP4_D3"
  norm.filtered.subset.long$sample[which(str_detect(norm.filtered.subset.long$condition, "H1_gt70_D8_BAP"))] <-
    "BAP_Yabe"
  norm.filtered.subset.long$sample <-
    factor(
      norm.filtered.subset.long$sample,
      levels = c("Amnion",
                 "CT",
...
vlnPlot <- function(info_table = Infotable) {
  ...
  ggplot(data = norm.filtered.subset.long,
         aes(x = sample, y = norm.expression, fill = sample)) +
    geom_flat_violin(position = position_nudge(x = 0.2, y = 0),
                     alpha = 0.8,
                     trim = FALSE) +
    geom_point(
      aes(y = norm.expression, color = sample),
...
Amnion markers genes using Roost dataset (from Io _et al_.,)
...
fig6b <- read.csv(file1, sep="\t", header = TRUE, stringsAsFactors = FALSE)
fig6b <- merge(
  fig6b,
  annot,
  by.x = "genes",
...
Money amnion markers
```{r markers3, warnings=TRUE, message=FALSE}
file2 <- "~/TutejaLab/amnion_for_ms_20210715/heatmaps.v2/Fig6b_and_monkeygenes/genes-monkey.txt"
monkey <- read.csv(file2, sep="\t", header = TRUE, stringsAsFactors = FALSE)
monkey <- merge(
  monkey,
  annot,
  by.x = "genes",
...
amnion.roost.new1 <- getTable(amnion.roost.names)
amnion.roost.old1 <- getTable(fig6b)
amnion.monkey1 <- getTable(monkey)
```

```{r tableout2, warnings=TRUE, message=FALSE}
amnion.roost.new2 <- addSampleInfo(amnion.roost.new1)
amnion.roost.old2 <-addSampleInfo(amnion.roost.old1)
amnion.monkey2 <- addSampleInfo(amnion.monkey1)
```


## Visulaization (new Amnion marker genes)


```{r hm1a, fig.cap="Fig 4.1: Heatmap for the new amnion marker genes", fig.width=8, fig.height=20}
runheatmap(amnion.roost.new1)
```

```{r cor1a, fig.cap="Fig 4.2: Corelation plot for the new amnion marker genes", fig.width=8, fig.height=8}
corPlot(amnion.roost.new1)
```
```{r normeach1a, fig.cap="Fig 4.2: distribution plot for the new amnion marker genes", fig.width=8, fig.height=8}
normTestEach(amnion.roost.new1)
```
```{r normall1a, fig.cap="Fig 4.2: distribution plot for the new amnion marker genes", fig.width=8, fig.height=8}
normTestAll(amnion.roost.new1)
```
...
```
```{r dt2a, warnings=TRUE, message=FALSE}
amnion.roost.new1 %>%  shapiro_test(norm.expression)
```
...
dunnTest <- dunnTest(norm.expression ~ sample, data=amnion.roost.new2, method="bh")
g <- as.data.frame(dunnTest$res)
DT::datatable(g)
```
```{r vln1a, fig.cap="Fig 4.2: Violin plot for the new amnion marker genes", fig.width=8, fig.height=8}
vlnPlot(amnion.roost.new2)
```
  

## Visulaization (old Amnion marker genes)


```{r hm1b, fig.cap="Fig 4.1: Heatmap for the new amnion marker genes", fig.width=8, fig.height=20}
runheatmap(amnion.roost.old1)
```

```{r cor1b, fig.cap="Fig 4.2: Corelation plot for the new amnion marker genes", fig.width=8, fig.height=8}
corPlot(amnion.roost.old1)
```
```{r normeach1b, fig.cap="Fig 4.2: distribution plot for the new amnion marker genes", fig.width=8, fig.height=8}
normTestEach(amnion.roost.old1)
```
```{r normall1b, fig.cap="Fig 4.2: distribution plot for the new amnion marker genes", fig.width=8, fig.height=8}
normTestAll(amnion.roost.old1)
```
```{r dt1b, warnings=TRUE, message=FALSE}
shapiro.each <- amnion.roost.old1 %>% dplyr::group_by(condition) %>% shapiro_test(norm.expression)
DT::datatable(shapiro.each)
```
```{r dt2b, warnings=TRUE, message=FALSE}
amnion.roost.old1 %>%  shapiro_test(norm.expression)
```
```{r dt3b, warnings=TRUE, message=FALSE}
kruskal.test(norm.expression ~ sample, data = amnion.roost.old2)
```
```{r dt4b, warnings=TRUE, message=FALSE}
dunnTest <- dunnTest(norm.expression ~ sample, data=amnion.roost.old2, method="bh")
g <- as.data.frame(dunnTest$res)
DT::datatable(g)
```
```{r vln1b, fig.cap="Fig 4.2: Violin plot for the new amnion marker genes", fig.width=8, fig.height=8}
vlnPlot(amnion.roost.old2)
```
  



## Visulaization (monkey amnion genes)


```{r hm1c, fig.cap="Fig 4.1: Heatmap for the new amnion marker genes", fig.width=8, fig.height=20}
runheatmap(amnion.monkey1)
```

```{r cor1c, fig.cap="Fig 4.2: Corelation plot for the new amnion marker genes", fig.width=8, fig.height=8}
corPlot(amnion.monkey1)
```
```{r normeach1c, fig.cap="Fig 4.2: distribution plot for the new amnion marker genes", fig.width=8, fig.height=8}
normTestEach(amnion.monkey1)
```
```{r normall1c, fig.cap="Fig 4.2: distribution plot for the new amnion marker genes", fig.width=8, fig.height=8}
normTestAll(amnion.monkey1)
```
```{r dt1c, warnings=TRUE, message=FALSE}
shapiro.each <- amnion.monkey1 %>% dplyr::group_by(condition) %>% shapiro_test(norm.expression)
DT::datatable(shapiro.each)
```
```{r dt2c, warnings=TRUE, message=FALSE}
amnion.monkey1 %>%  shapiro_test(norm.expression)
```
```{r dt3c, warnings=TRUE, message=FALSE}
kruskal.test(norm.expression ~ sample, data = amnion.monkey2)
```
```{r dt4c, warnings=TRUE, message=FALSE}
dunnTest <- dunnTest(norm.expression ~ sample, data=amnion.monkey2, method="bh")
g <- as.data.frame(dunnTest$res)
DT::datatable(g)
```
```{r vln1c, fig.cap="Fig 4.2: Violin plot for the new amnion marker genes", fig.width=8, fig.height=8}
vlnPlot(amnion.monkey2)
```
